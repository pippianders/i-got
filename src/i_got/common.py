    ...
    **kwargs
    ...
    while continue_renameing:
        ...
        if os.path.exists(filepath):
            if not force and (
                file_size == os.path.getsize(filepath) or skip_existing_file_size_check
            ):
                if not is_part:
                    if bar:
                        bar.done()
            ...
            with open(temp_filepath, open_mode) as output:
                while True:
                    ...
                    if not buffer:
                        ...
                        tmp_headers["Range"] = (
                            "bytes=" + str(received - chunk_start) + "-"
                        )
                        response = urlopen_with_retry(
    ...
    **kwargs
    ...
    if total_size:
        if (
            ...
            and not auto_rename
            and (
                os.path.getsize(output_filepath) >= total_size * 0.9
...
def script_main(download, download_playlist, **kwargs):
    ...
    playlist_grp.add_argument("--first", metavar="FIRST", help="the first number")
    playlist_grp.add_argument("--last", metavar="LAST", help="the last number")
    playlist_grp.add_argument(
        "--size", "--page-size", metavar="PAGE_SIZE", help="the page size number"
    )
    ...
    try:
        ...
        if not args.debug:
            ...
            log.e("  (2) Make sure i-got is up-to-date.")
            log.e("  (3) Check if the issue is already known, on")
            log.e("        https://github.com/pippianders/i-got/wiki/Known-Bugs")
            log.e("        https://github.com/pippianders/i-got/issues")
...
def google_search(url):
    ...
    videos = re.findall(r"(https://www\.youtube\.com/watch\?v=[\w-]+)", page)
...
def url_to_module(url):
    ...
    k = r1(r"([^.]+)", domain)
    if k in SITES:
        return (import_module(".".join(["i_got", "extractors", SITES[k]])), url)
    else:
        try:
