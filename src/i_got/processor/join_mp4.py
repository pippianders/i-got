...
def read_stsd(stream, size, left, type):
    ...
    class stsd_atom(Atom):
        ...
        def calsize(self):
            ...
            assert oldsize == self.size, "%s: %d, %d" % (
                self.type,
                oldsize,
                self.size,
            )  # TODO: remove
...
def read_avc1(stream, size, left, type):
    ...
    assert child.type in (b"avcC", b"pasp"), (
        "if the sub atom is not avcC or pasp (actual %s), you should not cache raw body"
        % child.type
    )
    left -= child.size
...
def read_stsc(stream, size, left, type):
    ...
    class stsc_atom(Atom):
        ...
        def write(self, stream):
            ...
            write_uint(stream, self.body[0])
            write_uint(stream, len(self.body[1]))
            for first_chunk, samples_per_chunk, sample_description_index in self.body[
                1
            ]:
...
def merge_moov(moovs, mdats):
    ...
    stts0 = merge_stts(
        x.get(b"mdia", b"minf", b"stbl", b"stts").body[1] for x in trak0s
    )
    stts1 = merge_stts(
        x.get(b"mdia", b"minf", b"stbl", b"stts").body[1] for x in trak1s
    )

    stss = merge_stss(
        (x.get(b"mdia", b"minf", b"stbl", b"stss").body[1] for x in trak0s),
        (len(x.get(b"mdia", b"minf", b"stbl", b"stsz").body[3]) for x in trak0s),
    )

    stsc0 = merge_stsc(
        (x.get(b"mdia", b"minf", b"stbl", b"stsc").body[1] for x in trak0s),
        (len(x.get(b"mdia", b"minf", b"stbl", b"stco").body[1]) for x in trak0s),
    )
    stsc1 = merge_stsc(
        (x.get(b"mdia", b"minf", b"stbl", b"stsc").body[1] for x in trak1s),
        (len(x.get(b"mdia", b"minf", b"stbl", b"stco").body[1]) for x in trak1s),
    )

    stco0 = merge_stco(
        (x.get(b"mdia", b"minf", b"stbl", b"stco").body[1] for x in trak0s), mdats
    )
    stco1 = merge_stco(
        (x.get(b"mdia", b"minf", b"stbl", b"stco").body[1] for x in trak1s), mdats
    )

    stsz0 = merge_stsz(
        (x.get(b"mdia", b"minf", b"stbl", b"stsz").body[3] for x in trak0s)
    )
    stsz1 = merge_stsz(
        (x.get(b"mdia", b"minf", b"stbl", b"stsz").body[3] for x in trak1s)
    )
