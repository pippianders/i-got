...
class AcFun(VideoExtractor):
    ...
    def prepare(self, **kwargs):
        assert re.match(
            r"https?://[^\.]*\.*acfun\.[^\.]+/(\D|bangumi)/\D\D(\d+)", self.url
        )
        ...
        if re.match(r"https?://[^\.]*\.*acfun\.[^\.]+/\D/\D\D(\d+)", self.url):
            ...
            if len(video_list) > 1:
                self.title += (
                    " - "
                    + [p.get("title") for p in video_list if p.get("id") == vid][0]
                )
        ...
        for stream in stream_list:
            ...
            stream_data = dict(
                src=m3u8_url, size=size, container=container, quality=quality
            )
    ...
    def acfun_download(
        self, url, output_dir=".", merge=True, info_only=False, **kwargs
    ):
        ...
        def getM3u8UrlFromCurrentVideoInfo(currentVideoInfo):
            if "playInfos" in currentVideoInfo:
                ...
                representation = ksPlayJson.get("adaptationSet")[0].get(
                    "representation"
                )
                reps = []
                for one in representation:
                    reps.append(
                        (one["width"] * one["height"], one["url"], one["backupUrl"])
                    )
                return max(reps)[1]
