...
def letvcloud_download_by_vu(
    vu, uu, title=None, output_dir=".", merge=True, info_only=False
):
    ...
    urls = [
        base64.b64decode(
            sorted(type_available, key=lambda x: x["video_quality"])[-1]["video_url"]
        ).decode("utf-8")
    ]
    size = urls_size(urls)
    ext = "mp4"
...
def letv_download(url, output_dir=".", merge=True, info_only=False, **kwargs):
    ...
    if re.match(r"http://yuntv.letv.com/", url):
        ...
        html = get_content(url)
        vid = (
            match1(url, r"http://www.letv.com/ptv/vplay/(\d+).html")
            or match1(url, r"http://www.le.com/ptv/vplay/(\d+).html")
            or match1(html, r'vid="(\d+)"')
        )
