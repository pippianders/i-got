...
url_patterns = [
    ...
    (pattern_url_group, "groups.pools.getPhotos", "group_id", get_NSID, "photos"),
    # www.flickr.com/photos/{username|NSID}/favorites/*
    (pattern_url_favorite, "favorites.getList", "user_id", get_NSID, "photos"),
]
...
def flickr_download_main(url, output_dir=".", merge=False, info_only=False, **kwargs):
    ...
    for url in urls:
        ...
        if not info_only:
            suffix = "[%d]" % index
            download_urls(
                [url], title + suffix, ext, False, output_dir, None, False, False
            )
            index = index + 1
...
def get_orig_video_source(api_key, pid, secret):
    parsed = json.loads(
        get_content_headered(tmpl_api_call_video_info % (api_key, pid, secret))
    )
