...
def icourses_download(url, output_dir=".", **kwargs):
    if "showResDetail.action" in url:
        ...
        url = "http://www.icourses.cn/jpk/changeforVideo.action?resId={}&courseId={}".format(
...
def public_course_get_title(url, page=None):
    ...
    return "{}_第{}讲_{}".format(
        course_main_title, seq_num + 1, public_course_playlist(url, page)[seq_num][1]
    )
...
def icourses_playlist_download(url, output_dir=".", **kwargs):
    ...
    ep = (
        "http://www.icourses.cn/jpk/viewCharacterDetail.action?sectionId={}&courseId={}"
    )
    change_for_video_ip = (
        "http://www.icourses.cn/jpk/changeforVideo.action?resId={}&courseId={}"
    )
    video_list = []
    ...
    if "viewVCourse" in url:
        ...
        if page_navi_vars is None:  # type 2 shared course
            ...
            sec_page = get_content(
                ep.format(page_navi_vars.group(2), page_navi_vars.group(1))
            )
...
class ICousesExactor(object):
    ...
    def basic_extract(self):
        ...
        self.get_flashvars()
        api_req_url = "{}?{}".format(
            self.flashvars["IService"], parse.urlencode(self.flashvars)
        )
    ...
    def get_sign(self, media_url):
        ...
        ssl_callback = get_content(
            "http://{}/ssl/ssl.shtml?r={}".format(media_host, ran)
        ).split(",")
        ssl_ts = int(
            datetime.datetime.strptime(ssl_callback[1], "%b %d %H:%M:%S %Y").timestamp()
            + int(ssl_callback[0])
        )
        sign_this = (
            self.__class__.ENCRYPT_SALT + parse.urlparse(media_url).path + str(ssl_ts)
        )
        arg_h = base64.b64encode(
            hashlib.md5(bytes(sign_this, "utf-8")).digest(), altchars=b"-_"
        )
...
def url_save_icourses(
    ...
    def dyn_update_url(received):
        if callable(dyn_callback):
            logging.debug(
                "Calling callback %s for new URL from %s"
                % (dyn_callback.__name__, received)
            )
    ...
    if os.path.exists(filepath):
        if not force:
            if not is_part:
                bar.done()
                print(
                    "Skipping %s: file already exists" % tr(os.path.basename(filepath))
                )
            else:
    ...
    assert received == os.path.getsize(temp_filepath), "%s == %s" % (
        received,
