...
fmt_level = dict(
    zip([str(codec["itag"]) for codec in youtube_codecs], range(len(youtube_codecs)))
)
...
def google_download(url, output_dir=".", merge=True, info_only=False, **kwargs):
    ...
    if service == "plus":  # Google Plus
        ...
        try:
            url = "https://plus.google.com/" + r1(
                r"(photos/\d+/albums/\d+/\d+)\?authkey", html
            )
            ...
            if post_author:
                post_url = "https://plus.google.com/+%s/posts/%s" % (
                    parse.quote(post_author),
                    r1(r"posts/(.+)", post_url),
                )
        ...
        real_url = (
            "https://docs.google.com/uc?export=download&confirm=no_antivirus&id=%s"
            % docid
        )
