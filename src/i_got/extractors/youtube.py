...
class YouTube(VideoExtractor):
    ...
    stream_types = [
        ...
        },
        {
            "itag": "6",
            "container": "FLV",
            "video_resolution": "270p",
            "video_encoding": "Sorenson H.263",
            "video_profile": "",
            "video_bitrate": "0.8",
            "audio_encoding": "MP3",
            "audio_bitrate": "64",
    ...
    def s_to_sig(js, s):
        ...
        js = js.replace("\n", " ")
        f1 = (
            match1(js, r"\.set\(\w+\.sp,encodeURIComponent\(([$\w]+)")
            or match1(js, r"\.set\(\w+\.sp,\(0,window\.encodeURIComponent\)\(([$\w]+)")
            or match1(js, r"\.set\(\w+\.sp,([$\w]+)\(\w+\.s\)\)")
            or match1(js, r'"signature",([$\w]+)\(\w+\.\w+\)')
            or match1(js, r"=([$\w]+)\(decodeURIComponent\(")
        )
        f1def = match1(js, r"function %s(\(\w+\)\{[^\{]+\})" % re.escape(f1)) or match1(
            js, r"\W%s=function(\(\w+\)\{[^\{]+\})" % re.escape(f1)
        )
    ...
    def get_vid_from_url(url):
        ...
        return (
            ...
            or match1(url, r"youtube\.com/shorts/([^/?]+)")
    ...
    def get_playlist_id_from_url(url):
        ...
        return parse_query_param(url, "list") or parse_query_param(url, "p")
    ...
    def download_playlist_by_url(self, url, **kwargs):
        ...
        playlist_json_serialized = match1(
            video_page,
        ...
        itemSection0 = tab0["tabRenderer"]["content"]["sectionListRenderer"][
            "contents"
        ][0]
    ...
    def prepare(self, **kwargs):
        ...
        if re.search("\Wlist=", self.url) and not kwargs.get("playlist"):
            log.w(
                "This video is from a playlist. (use --playlist to download all videos in the playlist.)"
            )
        ...
        if "status" not in video_info:
            ...
            if "use_cipher_signature" not in video_info or video_info[
                "use_cipher_signature"
            ] == ["False"]:
                self.title = parse.unquote_plus(
                    json.loads(video_info["player_response"][0])["videoDetails"][
                        "title"
                    ]
                )
                # Parse video page (for DASH)
                video_page = get_content(
                    "https://www.youtube.com/watch?v=%s" % self.vid
                )
                try:
                    try:
                        ...
                        if "assets" in ytplayer_config:
                            ...
                            self.html5player = "https://www.youtube.com" + re.search(
                                '([^"]*/base\.js)"', video_page
                            ).group(1)
                        ...
                        stream_list = ytInitialPlayerResponse["streamingData"][
                            "formats"
                        ]
                    ...
                    if "url_encoded_fmt_stream_map" not in video_info:
                        stream_list = json.loads(video_info["player_response"][0])[
                            "streamingData"
                        ]["formats"]
                    else:
            ...
            if video_info["errorcode"] == ["150"]:
                ...
                try:
                    ...
                    log.wtf(
                        '[Failed] Got message "%s". Try to login with --cookies.'
                        % msg.strip()
                    )
                ...
                if "title" in ytplayer_config["args"]:
                    ...
                    self.html5player = (
                        "https://www.youtube.com" + ytplayer_config["assets"]["js"]
                    )
                    stream_list = ytplayer_config["args"][
                        "url_encoded_fmt_stream_map"
                    ].split(",")
                ...
                log.wtf(
                    "[Failed] This video does not exist.", exit_code=None
                )  # int(video_info['errorcode'][0])
                raise

            else:
                log.wtf(
                    "[Failed] %s" % video_info["reason"][0], exit_code=None
                )  # int(video_info['errorcode'][0])
                raise
        ...
        if ytplayer_config and (
            ...
            if "hlsvp" in ytplayer_config["args"]:
                ...
                log.e(
                    "[Failed] %s" % player_response["playabilityStatus"]["reason"],
                    exit_code=1,
                )
        ...
        for stream in stream_list:
            if isinstance(stream, str):
                ...
                if "signatureCipher" in stream:
                    self.streams[stream_itag].update(
                        dict(
                            [
                                (_.split("=")[0], parse.unquote(_.split("=")[1]))
        ...
        try:
            try:
                caption_tracks = json.loads(ytplayer_config["args"]["player_response"])[
                    "captions"
                ]["playerCaptionsTracklistRenderer"]["captionTracks"]
            except:
                caption_tracks = ytInitialPlayerResponse["captions"][
        ...
        try:
            ...
            for aset in dash_xml.getElementsByTagName("AdaptationSet"):
                ...
                if mimeType == "audio/mp4":
                    ...
                    for rep in aset.getElementsByTagName("Representation"):
                        ...
                        if not dash_size:
                            try:
                                ...
                                continue
                        dash_urls = self.__class__.chunk_by_range(
                            dash_url, int(dash_size)
                        )
                        dash_mp4_a_urls = self.__class__.chunk_by_range(
                            dash_mp4_a_url, int(dash_mp4_a_size)
                        )
                    ...
                    for rep in aset.getElementsByTagName("Representation"):
                        ...
                        dash_webm_a_urls = self.__class__.chunk_by_range(
                            dash_webm_a_url, int(dash_webm_a_size)
                        )
                        self.dash_streams[itag] = {
                            "quality": "%sx%s" % (w, h),
                            "itag": itag,
            ...
            try:
                ...
                streams = [
                    dict(
                        [
                            (i.split("=")[0], parse.unquote(i.split("=")[1]))
                            for i in afmt.split("&")
                        ]
                    )
                ...
                if "adaptive_fmts" in video_info:
                    ...
                    try:
                        try:
                            streams = json.loads(video_info["player_response"][0])[
                                "streamingData"
                            ]["adaptiveFormats"]
                        except:
                    ...
                    streams = [
                        stream for stream in streams if "contentLength" in stream
                    ]
                    ...
                    for stream in streams:
                        ...
                        stream["init"] = "{}-{}".format(
                            stream["initRange"]["start"], stream["initRange"]["end"]
                        )
                        stream["index"] = "{}-{}".format(
                            stream["indexRange"]["start"], stream["indexRange"]["end"]
                        )
                        del stream["mimeType"]
            ...
            for stream in streams:  # video
                if "size" in stream:
                    if stream["type"].startswith("video/mp4"):
                        ...
                        self.dash_streams[itag] = {
                            "quality": "%s (%s)"
                            % (stream["size"], stream["quality_label"]),
                            "itag": itag,
                            "type": mimeType,
                            "mime": mimeType,
                            "container": "mp4",
                        ...
                        audio_urls = self.__class__.chunk_by_range(
                            audio_url, int(audio_size)
                        )
                        self.dash_streams[itag] = {
                            "quality": "%s (%s)"
                            % (stream["size"], stream["quality_label"]),
