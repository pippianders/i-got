...
def sohu_download(
    url, output_dir=".", merge=True, info_only=False, extractor_proxy=None, **kwargs
):
    ...
    info = json.loads(
        get_decoded_html("http://hot.vrs.sohu.com/vrs_flash.action?vid=%s" % vid)
    )
    if info and info.get("data", ""):
        for qtyp in ["oriVid", "superVid", "highVid", "norVid", "relativeId"]:
            ...
            if hqvid != 0 and hqvid != vid:
                info = json.loads(
                    get_decoded_html(
                        "http://hot.vrs.sohu.com/vrs_flash.action?vid=%s" % hqvid
                    )
                )
        ...
        info = json.loads(
            get_decoded_html(
                "http://my.tv.sohu.com/play/videonew.do?vid=%s&referer=http://my.tv.sohu.com"
                % vid
            )
        )
        host = info["allot"]
