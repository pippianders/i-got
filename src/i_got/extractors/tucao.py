...
def tucao_single_download(
    type_link, title, output_dir=".", merge=True, info_only=False
):
    ...
    # fix for 189 video source, see raw list types 7
        ...
        u = "http://www.tucao.tv/api/playurl.php?{}&key=tucao{:07x}.cc&r={}".format(
            type_link, random.getrandbits(28), int(time.time() * 1000)
        )
    ...
    if len(raw_l) == 1:
        ...
        for i in raw_l:
            ...
            tucao_single_download(
                format_link, title + "-" + sub_title, output_dir, merge, info_only
            )
