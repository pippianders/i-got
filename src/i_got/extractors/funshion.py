...
class Funshion(VideoExtractor):
    ...
    def dec_playinfo(cls, info, coeff):
        ...
        if cls.checksum(clear):
            res = dict(
                hashid=clear[:40], token=cls.funshion_decrypt_str(info["token"], coeff)
            )
        else:
    ...
    def prepare(self, **kwargs):
        ...
        if "title" not in kwargs:
            url = (
                "http://pv.funshion.com/v5/video/profile/?id={}&cl=mweb&uc=111".format(
                    self.vid
                )
            )
        ...
        ep_url = (
            self.__class__.video_ep
            if "single_video" in kwargs
                ...
                for st in self.__class__.stream_types:
                    ...
                    if s_id == st["id"]:
                        ...
                        token = urllib.parse.quote(
                            base64.b64encode(clear_info["token"].encode("utf8"))
                        )
