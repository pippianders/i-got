...
def ximalaya_download_by_id(
    id, title=None, output_dir=".", info_only=False, stream_id=None
):
...
def ximalaya_download(url, output_dir=".", info_only=False, stream_id=None, **kwargs):
    ...
    ximalaya_download_by_id(
        id, output_dir=output_dir, info_only=info_only, stream_id=stream_id
    )


def ximalaya_download_page(
    playlist_url, output_dir=".", info_only=False, stream_id=None, **kwargs
):
...
def ximalaya_download_playlist(
    ...
    for page in pages:
        ximalaya_download_page(
            page, output_dir=output_dir, info_only=info_only, stream_id=stream_id
        )
...
def print_stream_info(stream_id):
    ...
    print("      size:        %s" % "N/A")
    print("    # download-with: i-got --itag=%s [URL]" % stream_id)
