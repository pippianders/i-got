...
def zhihu_download(url, output_dir=".", merge=True, info_only=False, **kwargs):
    ...
    if len(paths) < 3 and len(paths) < 6:
        raise TypeError(
            "URL does not conform to specifications, Support column and question only."
            "Example URL: https://zhuanlan.zhihu.com/p/51669862 or "
            "https://www.zhihu.com/question/267782048/answer/490720324"
        )

    if (
        "question" not in paths or "answer" not in paths
    ) and "zhuanlan.zhihu.com" not in paths:
        raise TypeError(
            "URL does not conform to specifications, Support column and question only."
            "Example URL: https://zhuanlan.zhihu.com/p/51669862 or "
            "https://www.zhihu.com/question/267782048/answer/490720324"
    ...
    for index, video_id in enumerate(
        matchall(html, [r'<a class="video-box" href="\S+video/(\d+)"'])
    ):
        try:
            video_info = json.loads(
                get_content(
                    r"https://lens.zhihu.com/api/videos/{}".format(video_id),
...
def zhihu_download_playlist(url, output_dir=".", merge=True, info_only=False, **kwargs):
    if "question" not in url or "answer" in url:  # question page
        raise TypeError(
            "URL does not conform to specifications, Support question only."
    ...
    videos_url = r"https://www.zhihu.com/api/v4/questions/{}/answers".format(
        question_id
    )
    try:
        ...
        raise TypeError(
            "Check whether the problem URL exists.Example URL: https://www.zhihu.com/question/267782048"
        )
    ...
    while 1:
        for data in questions["data"]:
            ...
            zhihu_download(
                "https://www.zhihu.com/question/{}/answer/{}".format(
                    question_id, data["id"]
                ),
                output_dir=output_dir,
                merge=merge,
        ...
        questions = json.loads(
            get_content(questions["paging"]["next"], headers=fake_headers)
        )
