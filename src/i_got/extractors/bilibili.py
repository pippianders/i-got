...
class Bilibili(VideoExtractor):
    ...
    def bilibili_audio_info_api(sid):
        return (
            "https://www.bilibili.com/audio/music-service-c/web/song/info?sid=%s" % sid
        )

    @staticmethod
    def bilibili_audio_menu_info_api(sid):
        return (
            "https://www.bilibili.com/audio/music-service-c/web/menu/info?sid=%s" % sid
        )
    ...
    def bilibili_bangumi_api(avid, cid, ep_id, qn=0, fnval=16):
        return (
            "https://api.bilibili.com/pgc/player/web/playurl?avid=%s&cid=%s&qn=%s&type=&otype=json&ep_id=%s&fnver=0&fnval=%s"
    ...
    def bilibili_live_api(cid):
        return (
            "https://api.live.bilibili.com/room/v1/Room/playUrl?cid=%s&quality=0&platform=web"
            % cid
        )
    ...
    def bilibili_live_room_info_api(room_id):
        return (
            "https://api.live.bilibili.com/room/v1/Room/get_info?room_id=%s" % room_id
        )
    ...
    def bilibili_space_video_api(mid, pn=1, ps=50):
        return (
            "https://api.bilibili.com/x/space/arc/search?mid=%s&pn=%s&ps=%s&tid=0&keyword=&order=pubdate&jsonp=jsonp"
            % (mid, pn, ps)
        )
    ...
    def prepare(self, **kwargs):
        ...
        try:
            html_content = get_content(
                self.url, headers=self.bilibili_headers(referer=self.url)
            )
        except:
        ...
        # redirect: watchlater
        if re.match(
            r"https?://(www\.)?bilibili\.com/watchlater/#/(av(\d+)|BV(\S+)/?)", self.url
        ):
        ...
        # redirect: bangumi.bilibili.com/anime -> bangumi/play/ep
            ...
            r"https?://(www\.)?bilibili\.com/bangumi/play/ss(\d+)", self.url
        ) or re.match(r"https?://bangumi\.bilibili\.com/anime/(\d+)/play", self.url):
            initial_state_text = match1(
                html_content, r"__INITIAL_STATE__=(.*?);\(function\(\)"
            )  # FIXME
        ...
        # redirect: festival
            ...
            self.url = "https://www.bilibili.com/video/%s" % match1(
                self.url, r"bvid=([^&]+)"
            )
            html_content = get_content(self.url, headers=self.bilibili_headers())
        ...
        if re.match(r"https?://(www\.)?bilibili\.com/audio/au(\d+)", self.url):
            ...
            html_content,
            r'<meta property="og:url" content="(https://www.bilibili.com/bangumi/play/[^"]+)"',
        ):
            sort = "bangumi"
            ...
            r"https?://(www\.)?bilibili\.com/video/(av(\d+)|(bv(\S+))|(BV(\S+)))",
            self.url,
        ):
            sort = "video"
        ...
        if sort == "video":
            ...
            playinfo_text = match1(
                html_content, r"__playinfo__=(.*?)</script><script>"
            )  # FIXME
            ...
            html_content_ = get_content(
                self.url, headers=self.bilibili_headers(cookie="CURRENT_FNVAL=16")
            )
            playinfo_text_ = match1(
                html_content_, r"__playinfo__=(.*?)</script><script>"
            )  # FIXME
            ...
            if "videoData" in initial_state:
                ...
                if pn > 1 and not kwargs.get("playlist"):
                    log.w(
                        "This is a multipart video. (use --playlist to download all parts.)"
                    )
                ...
                p = int(
                    match1(self.url, r"[\?&]p=(\d+)")
                    or match1(self.url, r"/index_(\d+)")
                    or "1"
                )  # use URL to decide p-number, not initial_state['p']
                if pn > 1:
                ...
                cid = initial_state["videoData"]["pages"][p - 1][
                    "cid"
                ]  # use p-number, not initial_state['videoData']['cid']
            else:
                # (festival video)
            ...
            if playinfo is not None:
                current_quality = (
                    playinfo["data"]["quality"] or None
                )  # 0 indicates an error, fallback to None
                if (
                    "accept_quality" in playinfo["data"]
                    and playinfo["data"]["accept_quality"] != []
                ):
            ...
            for qn in [120, 112, 80, 64, 32, 16]:
                ...
                if current_quality is None or qn < current_quality:
                    ...
                    api_content = get_content(
                        api_url, headers=self.bilibili_headers(referer=self.url)
                    )
            ...
            for playinfo in playinfos:
                ...
                if "dash" in playinfo["data"]:
                    ...
                    for video in playinfo["data"]["dash"]["video"]:
                        ...
                        size = self.url_size(
                            baseurl, headers=self.bilibili_headers(referer=self.url)
                        )

                        # find matching audio track
                        if playinfo["data"]["dash"]["audio"]:
                            audio_baseurl = playinfo["data"]["dash"]["audio"][0][
                                "baseUrl"
                            ]
                            ...
                            if not audio_size_cache.get(audio_quality, False):
                                audio_size_cache[audio_quality] = self.url_size(
                                    audio_baseurl,
                                    headers=self.bilibili_headers(referer=self.url),
                                )
                            size += audio_size_cache[audio_quality]

                            self.dash_streams[format_id] = {
                                "container": container,
                                "quality": desc,
                                "src": [[baseurl], [audio_baseurl]],
                                "size": size,
                            }
                        else:
                            self.dash_streams[format_id] = {
            ...
            for fnval in [8, 16]:
                for qn in [120, 112, 80, 64, 32, 16]:
                    ...
                    if qn != current_quality:
                        api_url = self.bilibili_bangumi_api(
                            avid, cid, ep_id, qn=qn, fnval=fnval
                        )
                        api_content = get_content(
                            api_url, headers=self.bilibili_headers(referer=self.url)
                        )
            ...
            for playinfo in playinfos:
                ...
                if "dash" in playinfo["result"]:
                    for video in playinfo["result"]["dash"]["video"]:
                        ...
                        quality = self.height_to_quality(
                            video["height"], video["id"]
                        ...
                        # find matching audio track
                        audio_baseurl = playinfo["result"]["dash"]["audio"][0][
                            "baseUrl"
                        ]
        ...
        # vc video
            ...
            # set video title
            self.title = "%s (%s)" % (
                api_playinfo["data"]["user"]["name"],
                api_playinfo["data"]["item"]["id"],
            )
    ...
    def prepare_by_cid(self, avid, cid, title, html_content, playinfo, playinfo_, url):
        ...
        for qn in [80, 64, 32, 16]:
            ...
            if best_quality is None or qn <= best_quality:
                ...
                if api_playinfo_data.get("quality"):
                    playinfos.append(
                        {"code": 0, "message": "0", "ttl": 1, "data": api_playinfo_data}
                    )
        if not playinfos:
            log.w(message)
            # use bilibili error video instead
    ...
    def download_playlist_by_url(self, url, **kwargs):
        ...
        if re.match(r"https?://(www\.)?bilibili\.com/bangumi/play/ep(\d+)", self.url):
            ...
            r"https?://(www\.)?bilibili\.com/bangumi/media/md(\d+)", self.url
        ) or re.match(r"https?://bangumi\.bilibili\.com/anime/(\d+)", self.url):
            sort = "bangumi_md"
        elif re.match(
            r"https?://(www\.)?bilibili\.com/video/(av(\d+)|bv(\S+)|BV(\S+))", self.url
        ):
            sort = "video"
        elif re.match(
            r"https?://space\.?bilibili\.com/(\d+)/channel/detail\?.*cid=(\d+)",
            self.url,
        ):
            sort = "space_channel"
        elif re.match(
            r"https?://space\.?bilibili\.com/(\d+)/channel/seriesdetail\?.*sid=(\d+)",
            self.url,
        ):
            ...
            r"https?://space\.?bilibili\.com/(\d+)/favlist\?.*fid=(\d+)", self.url
        ):
            sort = "space_favlist"
        ...
        if sort == "video":
            ...
            if pn == len(initial_state["videoData"]["pages"]):
                ...
                urlcontent = get_content(
                    "https://api.bilibili.com/x/player.so?" + parse.urlencode(params),
                    headers=self.bilibili_headers(
                        referer="https://www.bilibili.com/video/av{}".format(aid)
                    ),
                )
                graph_version = json.loads(
                    urlcontent[
                        urlcontent.find("<interaction>")
                        + 13 : urlcontent.find("</interaction>")
                    ]
                )["graph_version"]
                params = {
                    "aid": str(aid),
                ...
                self.prepare_by_cid(
                    aid,
                    initial_state["videoData"]["cid"],
                    initial_state["videoData"]["title"]
                    + ("P{}. {}".format(1, node_info["data"]["title"])),
                    html_content,
                    playinfo,
                    playinfo_,
                    url,
                ...
                for choice in node_info["data"]["edges"]["choices"]:
                    ...
                    if not choice["cid"] in download_cid_set:
                        ...
                        self.prepare_by_cid(
                            ...
                            + (
                                "P{}. {}".format(
                                    len(download_cid_set), choice["option"]
                                )
                            ),
                            html_content,
                            playinfo,
            ...
            while True:
                ...
                if (
                    len(video_list) < archives_info["data"]["page"]["total"]
            ...
            pc = favlist_info["data"]["info"]["media_count"] // len(
                favlist_info["data"]["medias"]
            )
            if (
                favlist_info["data"]["info"]["media_count"]
            ...
            pc = math.ceil(
                videos_info["data"]["page"]["count"] / videos_info["data"]["page"]["ps"]
            )
