...
def tudou_download_by_iid(iid, title, output_dir=".", merge=True, info_only=False):
    ...
    temp = max(
        [data[i] for i in data if "size" in data[i][0]],
...
def tudou_download(url, output_dir=".", merge=True, info_only=False, **kwargs):
    if "acfun.tudou.com" in url:  # wrong way!
        ...
        i_got.extractors.acfun.acfun_download(url, output_dir, merge, info_only)
...
def parse_playlist(url):
    ...
    return [
        (atitle + "-" + x["title"], str(x["itemId"]))
        for x in json.loads(get_html(url))["message"]
    ]
...
def parse_plist(url):
    ...
    plist_info = json.loads(
        get_content("http://www.tudou.com/crp/plist.action?lcode=" + lcode)
    )
