...
def cntv_download(url, **kwargs):
    if re.match(r"http://tv\.cntv\.cn/video/(\w+)/(\w+)", url):
        ...
        re.match(
            r"http://\w+\.cntv\.cn/(\w+/\w+/(classpage/video/)?)?\d+/\d+\.shtml", url
        )
        or re.match(r"http://\w+.cntv.cn/(\w+/)*VIDE\d+.shtml", url)
        or re.match(
            r"http://(\w+).cntv.cn/(\w+)/classpage/video/(\d+)/(\d+).shtml", url
        )
        or re.match(r"http(s)?://\w+.cctv.com/\d+/\d+/\d+/\w+.shtml", url)
        or re.match(r"http://\w+.cntv.cn/\d+/\d+/\d+/\w+.shtml", url)
    ):
