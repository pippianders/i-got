...
def mtv81_download(url, output_dir=".", merge=True, info_only=False, **kwargs):
    ...
    xml = parseString(
        get_content(
            "http://intl.esperanto.mtvi.com/www/xml/media/mediaGen.jhtml?uri={}&flashPlayer=LNX%2013,0,0,206&geo=CN&sid=123456".format(
                vid
            )
        )
    )
    ...
    if not info_only:
        ...
        download_rtmp_url(
            ...
            ext=ext,
            params={
                "--swfVfy": "http://media.mtvnservices.com/player/prime/mediaplayerprime.1.10.8.swf"
            },
