...
def zhanqi_download(url, output_dir=".", merge=True, info_only=False, **kwargs):
    ...
    if not (
        path.startswith("videos") or path.startswith("v2/videos")
    ):  # url = "https://www.zhanqi.tv/huashan?param_s=1_0.2.0"
        path_list = path.split("/")
        room_id = path_list[1] if path_list[0] == "topic" else path_list[0]
        zhanqi_live(
            room_id, merge=merge, output_dir=output_dir, info_only=info_only, **kwargs
        )
        ...
        zhanqi_video(
            video_id, merge=merge, output_dir=output_dir, info_only=info_only, **kwargs
        )


def zhanqi_live(room_id, merge=True, output_dir=".", info_only=False, **kwargs):
    api_url = "https://www.zhanqi.tv/api/static/v2.1/room/domain/{}.json".format(
        room_id
    )
    ...
    video_levels = base64.b64decode(json_data["flashvars"]["VideoLevels"]).decode(
        "utf8"
    )
