...
def veoh_download(url, output_dir=".", merge=False, info_only=False, **kwargs):
    ...
    veoh_download_by_id(
        item_id, output_dir=".", merge=False, info_only=info_only, **kwargs
    )


# ----------------------------------------------------------------------
def veoh_download_by_id(
    item_id, output_dir=".", merge=False, info_only=False, **kwargs
):
    """Source: Android mobile"""
    webpage_url = "http://www.veoh.com/m/watch.php?v={item_id}&quality=1".format(
        item_id=item_id
    )
