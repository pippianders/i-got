...
def netease_cloud_music_download(
    ...
    if "album" in url:
        ...
        new_dir = (
            output_dir + "/" + fs.legitimize("%s - %s" % (artist_name, album_name))
        )
        if not info_only:
        ...
        for i in j["album"]["songs"]:
            ...
            try:  # download lyrics
                ...
                l = loads(
                    get_content(
                        "http://music.163.com/api/song/lyric/?id=%s&lv=-1&csrf_token="
                        % i["id"],
                        headers={"Referer": "http://music.163.com/"},
                    )
                )
                netease_lyric_download(
                    i, l["lrc"]["lyric"], output_dir=new_dir, info_only=info_only
                )
            except:
                pass
        ...
        j = loads(
            get_content(
                "http://music.163.com/api/song/detail/?id=%s&ids=[%s]&csrf_token="
                % (rid, rid),
                headers={"Referer": "http://music.163.com/"},
            )
        )
        ...
        j = loads(
            get_content(
                "http://music.163.com/api/dj/program/detail/?id=%s&ids=[%s]&csrf_token="
                % (rid, rid),
                headers={"Referer": "http://music.163.com/"},
            )
        )
        netease_song_download(
            j["program"]["mainSong"], output_dir=output_dir, info_only=info_only
        )
        ...
        while True:
            j = loads(
                get_content(
                    "http://music.163.com/api/dj/program/byradio/?radioId=%s&ids=[%s]&csrf_token=&offset=%d"
                    % (rid, rid, offset),
                    headers={"Referer": "http://music.163.com/"},
                )
            )
        ...
        j = loads(
            get_content(
                "http://music.163.com/api/mv/detail/?id=%s&ids=[%s]&csrf_token="
                % (rid, rid),
                headers={"Referer": "http://music.163.com/"},
            )
        )
...
def netease_song_download(song, output_dir=".", info_only=False, playlist_prefix=""):
    title = "%s%s. %s" % (playlist_prefix, song["position"], song["name"])
    url_best = (
        "http://music.163.com/song/media/outer/url?id=" + str(song["id"]) + ".mp3"
    )
    """
...
def netease_download(url, output_dir=".", merge=True, info_only=False, **kwargs):
    ...
    if "music.163.com" in url:
        ...
        src = r1(r'<source src="([^"]+)"', html) or r1(
            r'<source type="[^"]+" src="([^"]+)"', html
        )
