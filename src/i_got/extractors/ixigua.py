...
def ixigua_download(
    url, output_dir=".", merge=True, info_only=False, stream_id="", **kwargs
):
    ...
    video_resource = video_info["anyVideo"]["gidInformation"]["packerData"]["video"][
        "videoResource"
    ]
    ...
    if video_list.get("dynamic_video", None):
        audio_url = base64.b64decode(
            video_list["dynamic_video"]["dynamic_audio_list"][0]["main_url"].encode(
                "utf-8"
            )
        ).decode("utf-8")
        ...
        streams.append(
            {
                ...
                "video_url": base64.b64decode(
                    dynamic_video["main_url"].encode("utf-8")
                ).decode("utf-8"),
                "audio_url": audio_url,
