...
def twitter_download(url, output_dir=".", merge=True, info_only=False, **kwargs):
    ...
    screen_name = (
        r1(r"twitter\.com/([^/]+)", url)
        or r1(r'data-screen-name="([^"]*)"', html)
        or r1(r'<meta name="twitter:title" content="([^"]*)"', html)
    )
    item_id = (
        r1(r"twitter\.com/[^/]+/status/(\d+)", url)
        or r1(r'data-item-id="([^"]*)"', html)
        or r1(r'<meta name="twitter:site:id" content="([^"]*)"', html)
    )
    ...
    try:
        ...
        api_content = get_content(
            api_url,
            headers={"authorization": authorization, "x-guest-token": guest_token},
        )
        ...
        if item_id not in info["globalObjects"]["tweets"]:
            ...
            log.w(
                "[Failed] "
                + info["timeline"]["instructions"][0]["addEntries"]["entries"][0][
                    "content"
                ]["item"]["content"]["tombstone"]["tombstoneInfo"]["richText"]["text"]
            )
            assert False

        elif "extended_entities" in info["globalObjects"]["tweets"][item_id]:
            # if the tweet contains media, download them
            media = info["globalObjects"]["tweets"][item_id]["extended_entities"][
                "media"
            ]
        ...
        log.w(
            "[Warning] Falling back to deprecated Twitter API. Extraction may be incomplete."
        )
    ...
    for medium in media:
        if "video_info" in medium:
            ...
            title = (
                item_id + "_" + medium["media_url_https"].split(".")[-2].split("/")[-1]
            )
