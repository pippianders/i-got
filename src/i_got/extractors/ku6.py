...
def ku6_download_by_id(id, title=None, output_dir=".", merge=True, info_only=False):
    data = json.loads(get_html("http://v.ku6.com/fetchVideo4Player/%s...html" % id))[
        "data"
    ]
    t = data["t"]
    f = data["f"]
...
def ku6_download(url, output_dir=".", merge=True, info_only=False, **kwargs):
    ...
    if match1(url, r"http://baidu.ku6.com/watch/(.*)\.html") is not None:
        ...
        patterns = [
            r"http://v.ku6.com/special/show_\d+/(.*)\.\.\.html",
    ...
    if id is None:
        ...
        if not info_only:
            download_urls(
                [video_url], title, "mp4", video_size, output_dir, merge=merge, **kwargs
            )
        return
