...
def universal_download(url, output_dir=".", merge=True, info_only=False, **kwargs):
    ...
    if content_type.startswith("text/html"):
        ...
        for url in set(urls):
            ...
            candies.append({"url": url, "title": title})
        ...
        filename = parse.unquote(url_trunk.split("/")[-1]) or parse.unquote(
            url_trunk.split("/")[-2]
        )
        title = ".".join(filename.split(".")[:-1]) or filename
        ...
        if not info_only:
            download_urls(
                [url], title, ext, size, output_dir=output_dir, merge=merge, faker=True
            )
        return
