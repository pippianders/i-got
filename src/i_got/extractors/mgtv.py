...
class MGTV(VideoExtractor):
    ...
    def tk2(self):
        return base64.urlsafe_b64encode(
            b"did=%s|ver=%s|pno=%s|clit=%d"
            % (self.did.encode(), self.ver.encode(), self.pno.encode(), time.time())
        ).decode("utf-8")[::-1]
    ...
    def get_mgtv_real_url(self, url):
        ...
        content = get_content(
            content["info"], headers={"Referer": self.url}
    ...
    def download_playlist_by_url(self, url, **kwargs):
        ...
        content_playlist = get_content(
            self.playlist_endpoint.format(video_id=self.vid, page=1)
        )
        ...
        for page in range(2, max_page + 1):
            content_playlist = get_content(
                self.playlist_endpoint.format(video_id=self.vid, page=page)
            )
            content_playlist = loads(content_playlist)
            for ep in content_playlist["data"]["list"]:
                self.download_by_url("https://www.mgtv.com" + ep["url"], **kwargs)
    ...
    def prepare(self, **kwargs):
        ...
        content_player = get_content(
            self.player_endpoint.format(did=self.did, video_id=self.vid, tk2=self.tk2())
        )
        ...
        content_source = get_content(
            self.source_endpoint.format(video_id=self.vid, tk2=self.tk2(), pm2=pm2)
        )
