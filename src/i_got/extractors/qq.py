...
def qq_download_by_vid(vid, title, output_dir=".", merge=True, info_only=False):
    ...
    for part in range(1, seg_cnt + 1):
        if fc_cnt == 0:
            ...
            part_format_id = video_json["vl"]["vi"][0]["cl"]["ci"][part - 1][
                "keyid"
            ].split(".")[1]
        ...
        if key_json.get("key") is None:
            ...
            url = "{}{}?vkey={}".format(
                video_json["vl"]["vi"][0]["ul"]["ui"][0]["url"], fn_pre + ".mp4", vkey
            )
        else:
            vkey = key_json["key"]
    ...
    if not info_only:
        download_urls(
            part_urls, title, ext, total_size, output_dir=output_dir, merge=merge
        )
...
def kg_qq_download_by_shareid(shareid, output_dir=".", info_only=False, caption=False):
    ...
    lyric_url = (
        "http://cgi.kg.qq.com/fcgi-bin/fcg_lyric?jsonpCallback=jsopgetlrcdata&outCharset=utf-8&ksongmid="
        + ksong_mid
    )
...
def qq_download(url, output_dir=".", merge=True, info_only=False, **kwargs):
    ...
    if re.match(r"https?://(m\.)?egame.qq.com/", url):
        ...
        qq_egame.qq_egame_download(
            url, output_dir=output_dir, merge=merge, info_only=info_only, **kwargs
        )
        return
    ...
    if "kg.qq.com" in url or "kg2.qq.com" in url:
        ...
        kg_qq_download_by_shareid(
            shareid, output_dir=output_dir, info_only=info_only, caption=caption
        )
        return
    ...
    if "kuaibao.qq.com/s/" in url:
    ...
    elif "kuaibao.qq.com" in url or re.match(
        ...
        if rurl:
            ...
            if vid == "undefined" or vid == "index":
                vid = ""
        vid = (
            vid if vid else url.split("/")[-1].split(".")[0]
        )  # https://v.qq.com/x/cover/ps6mnfqyrfo7es3/q0181hpdvo5.html?
        vid = (
            vid if vid else match1(content, r'vid"*\s*:\s*"\s*([^"]+)"')
        )  # general fallback
        if not vid:
