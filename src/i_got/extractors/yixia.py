...
def miaopai_download_by_smid(smid, output_dir=".", merge=True, info_only=False):
    """"""
    api_endpoint = "https://n.miaopai.com/api/aj_media/info.json?smid={smid}".format(
        smid=smid
    )
...
def yixia_miaopai_download_by_scid(scid, output_dir=".", merge=True, info_only=False):
    """"""
    api_endpoint = "http://api.miaopai.com/m/v2_channel.json?fillType=259&scid={scid}&vend=miaopai".format(
        scid=scid
    )
...
def yixia_xiaokaxiu_download_by_scid(scid, output_dir=".", merge=True, info_only=False):
    """"""
    api_endpoint = (
        "http://api.xiaokaxiu.com/video/web/get_play_video?scid={scid}".format(
            scid=scid
        )
    )
...
def yixia_download(url, output_dir=".", merge=True, info_only=False, **kwargs):
    ...
    if "n.miaopai.com" == hostname:
        ...
        scid = (
            match1(url, r"miaopai\.com/show/channel/([^.]+)\.htm")
            or match1(url, r"miaopai\.com/show/([^.]+)\.htm")
            or match1(url, r"m\.miaopai\.com/show/channel/([^.]+)\.htm")
            or match1(url, r"m\.miaopai\.com/show/channel/([^.]+)")
        )
