...
def _get_video_query_url(resourceID):
    ...
    conn.request(
        "GET",
        "/vplus/remote.do?method=query2&loginname=videocas&pwd=af1c7a4c5f77f790722f7cae474c37e281203765d423a23b&resource=%5B%7B%22resourceID%22%3A%22"
        + resourceID
        + "%22%2C%22on%22%3A1%2C%22time%22%3A600%2C%22eid%22%3A100%2C%22w%22%3A800%2C%22h%22%3A600%7D%5D&timeStamp="
        + str(int(time())),
        headers=headers,
    )
...
def _get_video_list(resourceID):
    ...
    conn.request(
        "GET",
        "/vplus/member/resource.do?isyulan=0&method=queryFlashXmlByResourceId&resourceId={resourceID}&randoms={randoms}".format(
...
def _ucas_get_url_lists_by_resourceID(resourceID):
    ...
    for video_type_id, video_urls in enumerate(url_lists):
        for k, path in enumerate(video_urls):
            url_lists[video_type_id][
                k
            ] = "http://210.76.211.10/vplus/member/resource.do?virtualPath={virtualPath}&method=getImgByStream&imgPath={path}".format(
...
def ucas_download_single(url, output_dir=".", merge=False, info_only=False, **kwargs):
    ...
    resourceID = re.findall(
        r'resourceID":"([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})',
        html,
    )[0]
...
def ucas_download_playlist(url, output_dir=".", merge=False, info_only=False, **kwargs):
    ...
    for part_path in parts:
        ucas_download(
            "http://v.ucas.ac.cn/course/" + part_path,
            output_dir=output_dir,
            merge=merge,
            info_only=info_only,
        )
