...
class BokeCC(VideoExtractor):
    ...
    def download_by_id(
        self, vid="", title=None, output_dir=".", merge=True, info_only=False, **kwargs
    ):
        """self, str->None
    ...
    def prepare(self, vid="", title=None, **kwargs):
        ...
        api_url = self.API_ENDPOINT + "servlet/playinfo?vid={vid}&m=0".format(
            vid=vid
        )  # return XML
        ...
        if title is None:
            self.title = "_".join(
                [
                    i.text
                    for i in self.tree.iterfind("video/videomarks/videomark/markdesc")
                ]
            )
        else:
            self.title = title
        ...
        for i in self.tree.iterfind("video/quality"):
            ...
            self.stream_types.append({"id": quality, "video_profile": i.attrib["desp"]})
            self.streams[quality] = {"url": url, "video_profile": i.attrib["desp"]}
            self.streams_sorted = [
                dict(
                    [("id", stream_type["id"])]
