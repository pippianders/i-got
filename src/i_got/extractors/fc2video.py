...
def fc2video_download_by_upid(
    ...
    api_base = "http://video.fc2.com/ginfo.php?upid={upid}&mimi={mimi}".format(
        upid=upid, mimi=makeMimi(upid)
    )
    ...
    if not info_only:
        download_urls(
            [video_url], title, ext, size, output_dir, merge=merge, headers=fake_headers
        )
