...
class Imgur(VideoExtractor):
    ...
    def prepare(self, **kwargs):
        if re.search(r"imgur\.com/a/", self.url):
            ...
            album = match1(content, r"album\s*:\s*({.*}),") or match1(
                content, r"image\s*:\s*({.*}),"
            )
            ...
            self.streams = {
                "original": {
                    "src": [
                        "http://i.imgur.com/%s%s" % (i["hash"], ext) for i in images
                    ],
                ...
                "thumbnail": {
                    "src": [
                        "http://i.imgur.com/%ss%s" % (i["hash"], ".jpg") for i in images
                    ],
                    "container": "jpg",
                },
            }
            ...
            self.streams = {
                "original": {"src": [self.url], "size": size, "container": container}
            }
            ...
            content = get_content(self.url)
            url = match1(
                content, r'meta property="og:video"[^>]+(https?://i.imgur.com/[^"?]+)'
            ) or match1(
                content, r'meta property="og:image"[^>]+(https?://i.imgur.com/[^"?]+)'
            )
            _, container, size = url_info(url)
            self.streams = {
                "original": {"src": [url], "size": size, "container": container}
            }
