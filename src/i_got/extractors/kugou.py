...
def kugou_download(url, output_dir=".", merge=True, info_only=False, **kwargs):
    if url.lower().find("5sing") != -1:
        ...
        return kugou_download_playlist(
...
def kugou_download_by_hash(url, output_dir=".", merge=True, info_only=False):
    ...
    if not album_id:
        album_id = 123
    html = get_html(
        "http://www.kugou.com/yy/index.php?r=play/getdata&hash={}&album_id={}&mid=123".format(
            hash_val, album_id
        )
    )
    j = loads(html)
...
def kugou_download_playlist(url, output_dir=".", merge=True, info_only=False, **kwargs):
    ...
    if url.lower().find("rank") != -1:
        ...
        for song in res:
            ...
            if not album_id:
                album_id = 123
            urls.append(
                "http://www.kugou.com/song/#hash=%s&album_id=%s" % (hash_val, album_id)
            )
    ...
    # album sample:   http://www.kugou.com/yy/album/single/1645030.html
        ...
        for v in json.loads(res):
            urls.append(
                "http://www.kugou.com/song/#hash=%s&album_id=%s"
                % (v["hash"], v["album_id"])
            )
